...
function upload(config, cb) {
  ...
  var directoryUploader = function (getS3Params) {
    return function (cb) {
  ...
  var uploadVersionedFiles = directoryUploader(function(localFile, stat, cb) {
    if (localFile.includes('/' + config.version + '/')) {
      return cb(null, {
  ...
  var uploadNonVersionedFiles = directoryUploader(function(localFile, stat, cb) {
    if (!localFile.includes('/' + config.version + '/')) {
      return cb(null, {
  ...
  async.series([uploadVersionedFiles, uploadNonVersionedFiles], cb);
}
